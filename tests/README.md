...
- Specs atuais: autenticação com campos de senha exibir/ocultar, banco sem turmas, frequência com saída por aula e seletor de período próprio, troca de visão, deslize do paginador, abas da Gestão com toque, deslize e teclado, responsividade (barra lateral, modal centralizado no celular e login simétrico) e tema de três opções.
